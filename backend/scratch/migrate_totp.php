...
echo "--- Eyewear System TOTP 2FA DB Migration ---\n";
...
try {
    ...
    $checkQuery = "SHOW COLUMNS FROM `user` LIKE 'two_factor_secret'";
    $stmt = $pdo->query($checkQuery);
    $columnExists = $stmt->rowCount() > 0;
    
    if ($columnExists) {
        echo "two_factor_secret column already exists in `user` table. Skipping.\n";
    } else {
        echo "Adding two_factor_secret column to `user` table...\n";
        $pdo->exec("ALTER TABLE `user` 
            ADD COLUMN `two_factor_secret` VARCHAR(100) NULL AFTER `two_factor_enabled`;
        ");
